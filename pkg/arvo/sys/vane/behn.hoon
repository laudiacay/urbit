...
++  call
  ...
  |=  $:  hen=duct :: cargo cult it for now, its about how arvo dispatches events
          ...
          wrapped-task=(hobo task) :: dumb little type hack??? if the event came from the runtime, it's untyped, if it came from a vane, it's typed
      ==
  ^-  [(list move) _behn-gate]
  ::
  =/  =task  ((harden task) wrapped-task) :: this tries to "harden" -> if its typed, nothing, if its untyped, you mold it into a "task" from lull. then assign that to variable task in subject- shadowing :)
  ...
  =^  moves  state :: this line is the "state monad" in that moves is "new/"pinned"
    ...
    ==
  [moves behn-gate] :: CR: you are producing a list of moves and an outer copy of the behn-gate
...
++  load
  ...
  ^+  behn-gate
  behn-gate(state old)
::  +scry: view timer state (namespace handler)
::
